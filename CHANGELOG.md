...
- **Launching Oh My Pi in the native terminal died with `&& was unexpected at this time`.** The terminal command bakes env vars into an inline `set` chain (`/K set A=1&&set B=2&&<binary>`); an agent with no env vars (omp has none) produced `/K &&omp`, which cmd rejects. The `&&` is now emitted only when the chain is non-empty. The external-console path was unaffected (env goes through `ProcessStartInfo`).
