...
### ✅ Roslyn-precise C# diagnostic ranges — shipped 1.16.0

`getDiagnostics` now upgrades every file the Error List flags (plus the requested file) to real start/end spans from the document's semantic model (`RoslynReader.GetPreciseDiagnosticsAsync`); C++/loose files keep the Error List point ranges, and a Roslyn-clean file keeps its Error List entries (build-only errors). Original analysis kept below:
...
- **Transitive callees — ✅ SHIPPED 1.17.0**: `vs_call_hierarchy direction:callees` now recurses (depth-capped, cycle-guarded, budget-signaled) exactly like callers; only in-solution callees recurse (framework targets are leaves).
- **Rename / safe refactors — ✅ SHIPPED 1.17.0** as `vs_rename`: Roslyn `Renamer.RenameSymbolAsync`, PREVIEW BY DEFAULT (per-file edit counts + sample lines); `apply=true` commits via `workspace.TryApplyChanges` = ONE VS undo unit (Ctrl+Z reverts the whole rename), failing cleanly if the solution changed mid-rename. (The originally sketched per-file diff-gate routing was traded for preview+apply+undo — simpler, and the undo unit gives the same take-it-back safety.)
...
- **Test-driven debugging loop — ✅ SHIPPED 1.10.0** ([`docs/TESTING.md`](docs/TESTING.md)). Discover/run/re-run-failed/debug tests through VS's Test Explorer engine, plus the flaky-hunter (`vs_hunt_flaky`) and **catch-red-handed** (`vs_catch_flaky` — loop a test under the debugger until the failing iteration halts at the throw). Remaining test follow-ups: **run tests affected by a change — ✅ SHIPPED 1.16.0** (`vs_run_affected`: Roslyn caller-graph BFS from the changed files' declarations up to test methods, then one multi-`Scope.ForSymbol` engine pass); **hunt idle-wait — ✅ SHIPPED 1.18.0** (`IOperationState` polled for real idle between runs, guarded reflection with fixed-delay fallback); **profiling — ⚙️ wired experimentally 1.18.0** (the Diagnostics Hub CPU Usage tool GUID is set on `TestRunOptions` via a name-scanned property, `profilerToolId` overrides; needs live verification) — original note on **profiling** (wire a Diagnostics-Hub `ProfilerToolId` so `profile:true` runs under the profiler); an **`IOperationState` engine-idle wait** between hunt runs so `measureRate` never under-samples on cancellation churn.
- **Tracepoints — ✅ SHIPPED 1.17.0** (simulated, exactly as sketched): `vs_set_tracepoint(file, line, expressions[], maxHits)` places a tagged breakpoint + an `OnEnterBreakMode` handler that evaluates the expressions, records `{seq, time, values}`, and sets `ExecutionAction=Go` (log-and-continue); `vs_get_tracepoint` polls the timeline, `vs_remove_tracepoint` disarms. Auto-disables after `maxHits`. `Debugging/TracepointService.cs`.
- **CPU / memory profiling — ✅ SHIPPED 1.17.0** (`Tools/PerfTools.cs`): `vs_perf_counters` (dotnet-counters sampling window → one line per counter), `vs_trace_cpu` (dotnet-trace cpu-sampling → top-N hot methods report), `vs_gc_dump` (dotnet-gcdump → top types by size; file kept for PerfView/VS). Each errors with an install hint (`dotnet tool install -g …`) when its CLI is missing.
- **Per-frame source — ✅ SHIPPED 1.18.0**, and WITHOUT AD7: `EnvDTE90a.StackFrame2` exposes `FileName`/`LineNumber` per frame (the same "the newer automation interface has it, just cast up" lesson as break-on-thrown). `vs_debug_state` frames carry `file`+`line`; `vs_threads` suffixes ` (file:line)`.
...
- **`vs_capture_window` / `vs_capture_screen` — ✅ SHIPPED 1.14.0** ([`docs/VISION.md`](docs/VISION.md)): debuggee window, VS window, window-by-title (the browser case), and full screen, ALL behind the "Allow screen capture" panel toggle, every capture staged as a visible tray chip + feed line, path returned (never MCP image blocks). Remaining capture follow-ups: **region-crop — ✅ SHIPPED 1.17.0** (`region:{x,y,width,height}` on both capture tools; out-of-bounds clamps, degenerate crop keeps the full image). Before/after pairs: not built as a tool — the model just calls capture twice around its action; a dedicated pair tool can't hold the "act" step anyway.
