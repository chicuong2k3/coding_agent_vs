...
- `src/ClaudeCodeVS/Tools/` - one `IIdeTool` per tool: the 12 IDE-protocol tools (openDiff, openFile, getDiagnostics, …) plus the debugger surface - `DebugTools.cs` (reads) + `DriveTools.cs` (gated drive) - plus the semantic surface - `SemanticTools.cs` (the 9 `vs-semantic` Roslyn tools, incl. `vs_get_selection` + `vs_decompile` + `vs_rename` — the one mutating one, preview-first) - plus the test surface - `TestTools.cs` (list/run/rerun-failed/debug/hunt/catch, on `vs-debug`).
...
- **Manifest targets `[17.14,)`** — DELIBERATELY unbounded above so future VS majors install without a re-release (user decision, 2026-08-08; do NOT "fix" it back to `[17.14,19.0)` — the Qodo compliance rule 2396913 still says the bounded range and should be updated instead). Extension is tested on VS 2026 only; VS 2022 verification is a future item - see `ROADMAP.md`.
...
The third knowledge axis (after runtime state and diagnostics): Roslyn's resolved **semantic model** of the code, exposed as 9 tools (8 read-only + `vs_rename`, the one mutating one — Roslyn `Renamer`, preview-by-default, `apply=true` = one VS undo unit via `TryApplyChanges`) so the model navigates by ground truth instead of grep. `vs_call_hierarchy` callees is transitive since 1.17.0 (was depth-1). Full reference: `docs/SEMANTIC.md`.
